...
void WebSocketTest::measureInternalThroughput(unsigned int payloadLength, int echoes, bool ssl) {
    ...
    h.onMessage([payload, payloadLength, closeMessage, closeMessageLength, echoes](uWS::WebSocket<uWS::CLIENT> *ws,
    ...
    h.onDisconnection([&h, closeMessage, closeMessageLength](uWS::WebSocket<uWS::SERVER> *ws, int code, char *message,
                                                             size_t length) {
    ...
    if (!ms) {
        ...
        std::cout << "Throughput: " << (float(echoes) / float(ms)) << " echoes/ms (" << ms << " ms elapsed)"
                  << std::endl;
    }
...
void WebSocketTest::testBroadcast() {
    ...
    h.onMessage([&broadcasts, broadcastMessage, broadcastMessageLength](uWS::WebSocket<uWS::CLIENT> *ws, char *message,
...
void WebSocketTest::testRouting() {
    ...
    h.listen(3000);
    h.connect("ws://localhost:3000/somePathHere", nullptr, {{"sec-websocket-protocol", "someSubProtocolHere"},
                                                            {"some-random-header",     "someRandomValue"}});
...
void WebSocketTest::testTransfers() {
    for (int ssl = 0; ssl < 2; ssl++) {
        ...
        std::thread t([&tServerGroup, &client, &receivedMessages, &clientGroup, &m] {
            ...
            th.onMessage([&tServerGroup, &client, &receivedMessages, &clientGroup, &m, &transferred](
...
void WebSocketTest::testSendCallback() {
    ...
    h.onConnection([&h](uWS::WebSocket<uWS::SERVER> *ws, uWS::HttpRequest req) {
        ws->send("12341111", 12, uWS::OpCode::TEXT,
                 [](uWS::WebSocket<uWS::SERVER> *ws, void *data, bool cancelled, void *reserved) {
                     if (data) {
                         if (data != (void *) 13) {
...
void WebSocketTest::testMessageBatch() {
    ...
    h.onConnection([&messages, &excludes](uWS::WebSocket<uWS::SERVER> *ws, uWS::HttpRequest req) {
        uWS::WebSocket<uWS::SERVER>::PreparedMessage *prepared = ws->prepareMessageBatch(messages, excludes,
    ...
    h.onMessage([&receivedMessages, &h](uWS::WebSocket<uWS::CLIENT> *ws, char *message, size_t length,
                                        uWS::OpCode opCode) {
...
void WebSocketTest::testHTTP() {
    ...
    auto controlData = [&h, &expectedRequests](uWS::HttpResponse *res, char *data, size_t length,
                                               size_t remainingBytes) {
        std::string *buffer = (std::string *) res->httpSocket->getUserData();
        buffer->append(data, length);

        std::cout << "HTTP POST, chunk: " << length << ", total: " << buffer->length() << ", remainingBytes: "
    ...
    h.onHttpRequest([&h, &expectedRequests, &controlData](uWS::HttpResponse *res, uWS::HttpRequest req, char *data,
...
void WebSocketTest::serveEventSource() {
    ...
    h.onHttpRequest([&h, &document, &header](uWS::HttpResponse *res, uWS::HttpRequest req, char *data, size_t length,
                                             size_t remainingBytes) {
...
void WebSocketTest::serveHttp() {
    ...
    h.onHttpRequest([&document](uWS::HttpResponse *res, uWS::HttpRequest req, char *data, size_t length,
                                size_t remainingBytes) {
...
void WebSocketTest::testReceivePerformance() {
    ...
    unsigned char webSocketMessage[] = {0x82, 0x9c, 0x37, 0x22, 0x79, 0xa5, 0x65, 0x4d, 0x1a, 0xce, 0x17, 0x4b, 0x0d,
                                        ...
                                        0x52, 0x40, 0x2a, 0xca, 0x54, 0x49, 0x1c, 0xd1};
...
//    ,0x1c, 0xd1};
    ...
    h.onConnection([originalBuffer, buffer, bufferLength, messages, &h](uWS::WebSocket<uWS::SERVER> *ws,
                                                                        uWS::HttpRequest req) {
        for (int i = 0; i < 100; i++) {
...
void WebSocketTest::testThreadSafety() {
    ...
    for (int ssl = 0; ssl < 2; ssl++) {
        ...
        h.onMessage([&sharedSocket, &holdUp, &remainingMessages, &h](uWS::WebSocket<uWS::CLIENT> *ws, char *message,
                                                                     size_t length, uWS::OpCode opCode) {
